...
console.log(isThisAnEmail("hilinawuhib@gmai"))
...
console.log(howManyDays(new Date(2021, 9, 27)))
...
function onlyInTheMillennium(movieArray) {
    let postYear2000=[]
    for(i=0;i<movieArray.length;i++){
        let movieRelease= movieArray[i].Year;
     if (movieRelease>1999){
            postYear2000=(movieArray[i])
...
console.log(onlyInTheMillennium(movies))
...
function sumAllTheYears(movieList) {
     let allYears = 0
  for ( i = 0; i < movieList.length ; i++){
       allYears+=parseInt(movieList[i].Year)
   }
  return allYears
...
for (let i = 0; i < movies.length; i++){
    let movie= movies[i].Title;
    if (movie.indexOf(str)!== -1){
        movieMatches.match.push(movies[i])
    }
    else{
        movieMatches.unmatch.push(movies[i])
    }
}return movieMatches
}
